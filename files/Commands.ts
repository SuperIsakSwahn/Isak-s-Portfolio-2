    ...
    async processMessage(steamID: SteamID, message: string): Promise<void> {
        ...
        if (message.startsWith(prefix)) {
            if (command === 'help') {
                ...
                this.buyFileCommand(steamID, message, command as Instant, prefix);
                ...
                this.describeCommand(steamID, message, prefix);
            ...
            } else if (['combinetoken', 'ctk', 'combinetokens'].includes(command) && isAdmin) {
                this.crafting.combineTokenCommand(steamID, message, -1, false);
            } else if (['crafthat', 'ch'].includes(command) && isAdmin) {
                this.crafting.craftHatsCommand(steamID, message);
            } else if (['craftscrap', 'cs', 'cscrap'].includes(command) && isAdmin) {
                this.crafting.craftScrapCommand(steamID, message);
            } else if (['craftrec', 'crec'].includes(command) && isAdmin) {
                this.crafting.craftRecCommand(steamID, message);
            } else if (['craftref', 'cref'].includes(command) && isAdmin) {
                this.crafting.craftRefCommand(steamID, message);
            } else if (['crafttoken', 'ct'].includes(command) && isAdmin) {
                this.crafting.craftTokenCommand(steamID, message);
            } else if (['craftvaccinator', 'cv'].includes(command) && isAdmin) {
                this.crafting.craftVaccinatorCommand(steamID, message); // Added message
        ...
        if (arg === '') {
            ...
            const list = Array.from(filesForSale.entries())
                .map(([name]) => {
                    ...
                    if (priceStr == '') priceStr = 'Free, and unsellable because it\'s not supposed to be free';
...
// Helper function to extract amount and item from input string
    private extractAmountAndItem(input: string): { amount?: number; item: string } {
        ...
        if (match) {
            return { amount: parseInt(match[1], 10), item: match[2].trim() };
        }

        match = input.match(/^(.+?)\s+(\d+)$/);
        if (match) {
            return { amount: parseInt(match[2], 10), item: match[1].trim() };
        }
